...
impl Widget for ColumnEditorWidget<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for (i, col) in visible_columns.iter().enumerate() {
            ...
            let enabled = if let Some(state) = self.state.config.states.iter().find(|s| s.column == **col) {
                state.enabled
            } else {
                col.default_enabled()
            };
        ...
        for col in x..x + modal_area.width {
            buf[(col, key_hints_y)].set_style(Style::default().bg(Color::Rgb(30, 30, 46)).fg(Color::Gray));
        }
        ...
        for (i, ch) in hints_text.chars().enumerate() {
            ...
            if pos < x + modal_area.width - 1 {
                buf[(pos, key_hints_y)].set_char(ch).set_style(Style::default().bg(Color::Rgb(30, 30, 46)).fg(Color::Gray));
            }
        }
        ...
        for col in x..x + modal_area.width {
            buf[(col, bottom_border_y)].set_style(Style::default().bg(Color::Rgb(30, 30, 46)).fg(Color::Gray));
        }
    }
}
