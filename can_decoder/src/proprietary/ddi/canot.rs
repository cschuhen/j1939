...
pub fn lookup(ddi: u16) -> Option<&'static DdiInfo> {
    PROPRIETARY_DDI_LIST
        .binary_search_by_key(&ddi, |info| info.ddi)
        .ok()
        .map(|idx| &PROPRIETARY_DDI_LIST[idx])
}
